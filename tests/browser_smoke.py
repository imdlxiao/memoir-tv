    ...
    with tempfile.TemporaryDirectory(prefix='memoir-test-') as directory:
        ...
        try:
            with sync_playwright() as p:
                ...
                # F5 must discover a move without requiring the manual scan button.
                (media / '20240715.png').rename(root / 'moved-photo.png')
                page.reload()
                expect(page.locator('.memory-card')).to_have_count(1)
                expect(page.locator('.card-title')).not_to_have_text('测试回忆')
                (root / 'moved-photo.png').rename(media / '20240715.png')
                expect(page.locator('.memory-card')).to_have_count(2, timeout=12000)
                expect(page.locator('.card-title').first).to_have_text('测试回忆')
                # An already-open page also notices moves and restores automatically.
                (media / 'unknown.mp4').rename(root / 'moved-video.mp4')
                expect(page.locator('.memory-card')).to_have_count(1, timeout=12000)
                (root / 'moved-video.mp4').rename(media / 'unknown.mp4')
                expect(page.locator('.memory-card')).to_have_count(2, timeout=12000)
