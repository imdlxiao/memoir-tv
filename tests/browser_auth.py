    ...
    with tempfile.TemporaryDirectory(prefix='memoir-auth-') as folder:
        ...
        try:
            with sync_playwright() as p:
                ...
                private_row = page.locator(f'[data-media="{ids["private.mp4"]}"]')
                private_row.locator('[name=scope]').select_option('admin')
                private_row.get_by_role('button', name='保存可见范围').click()
                expect(private_row.locator('[role=status]')).to_contain_text('已保存')
