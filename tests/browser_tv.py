...
def main():
    with tv_library() as (server, repo), sync_playwright() as p:
        ...
        for legacy in [False, True]:
            ...
            page.wait_for_function('document.querySelector("video").getBoundingClientRect().height > 120 && document.querySelector("#viewer-dialog").getBoundingClientRect().height > 240')
            page.set_viewport_size({'width': 1280, 'height': 600})
            page.wait_for_function('document.documentElement.style.getPropertyValue("--viewport-height") === "600px"')
            page.wait_for_function('document.querySelector("#viewer-dialog").getBoundingClientRect().bottom <= innerHeight + 1')
