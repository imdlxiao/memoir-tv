...
function render() {
  ...
  media.addEventListener('error', () => {
    const container = $('.viewer-media', dialog);
    container.innerHTML = `<div class="viewer-error"><p>浏览器暂时无法打开这份原片。部分 MOV、HEVC、HEIC 格式需要设备解码支持，可以下载后用本地播放器查看。</p><a href="${e(mediaURL(item))}" download="${e(item.filename)}">${item.kind === 'video' ? '下载视频原片' : '下载照片原片'}</a></div>`;
  });
  if (item.kind === 'video')
    media.addEventListener('ended', () => {
      if (document.body.classList.contains('tv-mode')) navigate(1);
    });
  else if (document.body.classList.contains('tv-mode'))
    slideshow = setTimeout(() => navigate(1), 8000);
