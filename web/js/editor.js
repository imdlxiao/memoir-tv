...
export function editMemory(item, onSaved) {
  ...
  const form = $('form', dialog), precision = form.elements.precision, date = form.elements.date;
  precision.value = item.precision || 'unknown';
  const syncDate = () => { const old = date.value || item.date || ''; date.type = precision.value === 'month' ? 'month' : 'date'; date.disabled = precision.value === 'unknown'; date.required = !date.disabled; date.value = date.disabled ? '' : precision.value === 'month' ? old.slice(0,7) : old.length === 10 ? old : ''; date.pattern = precision.value === 'month' ? '[0-9]{4}-[0-9]{2}' : '[0-9]{4}-[0-9]{2}-[0-9]{2}'; date.placeholder = precision.value === 'month' ? 'YYYY-MM' : 'YYYY-MM-DD'; };
  syncDate(); precision.addEventListener('change', syncDate);
  dialog.querySelectorAll('[data-close]').forEach(button => button.addEventListener('click',()=>dialog.close()));
  form.addEventListener('submit', async event => {
    event.preventDefault(); const error = $('.form-error', dialog), submit = $('[type=submit]', form); error.hidden = true;
    const changes = {title:form.elements.title.value.trim(),description:form.elements.description.value.trim(),precision:precision.value,date:date.value,location:form.elements.location.value.trim(),tags:[...new Set(form.elements.tags.value.split(/[,，;；\n]/).map(t=>t.trim().replace(/^#/, '')).filter(Boolean))]};
    if (changes.tags.length > 20 || changes.tags.some(t=>t.length > 30)) { error.textContent='最多 20 个标签，每个最多 30 个字。'; error.hidden=false; return; }
    submit.disabled = true;
    try { await saveMemory(item.id, changes); Object.assign(item, changes); dialog.close(); onSaved(); toast('这段回忆，已经好好收下了'); }
