...
export function editMemory(item, onSaved) {
  ...
  const coordinates = validCoordinates(item.coordinates) ? item.coordinates : null;
  form.elements.location
    .closest('label')
    .insertAdjacentHTML(
      'afterend',
      `<div class="coordinate-fields"><div class="form-row"><label class="form-field">纬度<input name="latitude" type="number" step="any" min="-90" max="90" value="${coordinates?.latitude ?? ''}" placeholder="-90 至 90"></label><label class="form-field">经度<input name="longitude" type="number" step="any" min="-180" max="180" value="${coordinates?.longitude ?? ''}" placeholder="-180 至 180"></label></div><div class="coordinate-actions"><button type="button" class="secondary-button" data-pick-location>${icon('pin')}地图选点</button><button type="button" class="quiet-button" data-clear-location>清除坐标</button></div><p class="form-hint">地名用于筛选，坐标用于地图。使用 WGS84 坐标；空白表示不在地图展示。</p></div>`,
    );
  const readCoordinates = () => {
    ...
    if (!latitude || !longitude || !validCoordinates(value))
      throw new Error('请同时填写有效的纬度和经度');
    return value;
  };
  $('[data-clear-location]', dialog).onclick = () => {
    form.elements.latitude.value = '';
    form.elements.longitude.value = '';
  };
  $('[data-pick-location]', dialog).onclick = () => {
    let initial;
    try {
      initial = readCoordinates();
    } catch {
      initial = null;
    }
    openMap([], {
      initial,
      onPick: (value) => {
        form.elements.latitude.value = value.latitude;
        form.elements.longitude.value = value.longitude;
        $('[data-pick-location]', dialog).focus();
      },
    });
  };
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      changes.coordinates = readCoordinates();
