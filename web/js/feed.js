...
function card(item) {
  const name = titleOf(item), isVideo = item.kind === 'video';
...
export function renderFeed() {
  const items = filteredItems(), feed = $('#feed');
  ...
  if (!items.length) {
    ...
    const messages = {photo:['相片的位置，给美好留着','把照片放进素材目录，刷新回忆库后就会出现在这里。'],favorites:['把舍不得的瞬间，珍藏起来','轻点回忆下的爱心，下次想念时就能更快找到。'],undated:['每段回忆，都找到了日期','所有回忆已有拍摄日期，继续去看看那些好时光吧。']};
    const [title, description] = filtered ? ['还没找到这个瞬间','试试其他关键词，或者清除筛选再看看。'] : messages[state.view === 'all' ? state.type : state.view] || ['时光簿，等你来填满','把视频或照片放进素材目录，再到管理回忆库中刷新。'];
  ...
  const chips = [['year',state.year === 'unknown' ? '日期待补充' : state.year],['location',state.location],['tag',state.tag]];
  $('#active-filters').innerHTML = chips.filter(([,value])=>value).map(([key,value])=>`<button class="filter-chip" data-clear="${key}">${e(value)} ×</button>`).join('');
  $('#filter-dot').hidden = !chips.some(([,value])=>value);
  feed.querySelectorAll('img').forEach(img => img.addEventListener('error', () => { const fallback = document.createElement('span'); fallback.className='media-fallback'; fallback.innerHTML=`${icon('image')}<span>预览暂不可用，点击打开原片</span>`; img.replaceWith(fallback); }, {once:true}));
}
